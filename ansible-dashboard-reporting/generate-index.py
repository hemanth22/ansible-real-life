...
def generate_html(metadata, grouped_dashboards):
    ...
    author_url = site.get('author_url', '')
...
<body>
    <div class="hero">
        <h1>🚀 {title}</h1>
        <p>{subtitle}<br>Maintained by {f'<a href="{author_url}" style="color: white; text-decoration: underline;">{author}</a>' if author_url else author}</p>
        <div class="stats">
            <div class="stat-item">
...
'''
    ...
    <footer>
        <p>Can be used with Red Hat Ansible Automation Platform</p>
        <p>Maintained by {f'<a href="{author_url}" target="_blank">{author}</a>' if author_url else author}</p>
        <a href="{github_repo}" class="github-link" target="_blank">View on GitHub</a>
        <p style="margin-top: 20px; font-size: 0.85em; opacity: 0.7;">
            © {datetime.now().year} | Last generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S UTC")}
        </p>
    </footer>
</body>
</html>
